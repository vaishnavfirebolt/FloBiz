...
import com.vaishnav.flobiz.databinding.ActivityMainBinding
import com.vaishnav.flobiz.hide
import com.vaishnav.flobiz.model.RecyclerViewData
import com.vaishnav.flobiz.show
import com.vaishnav.flobiz.ui.main.adapter.MainRecyclerViewAdapter
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        sharedPreferences.getBoolean(PREFS_KEY_IS_AD_CANCELLED, true).let {
            showAd = it
        }
        ...
        mainRecyclerViewAdapter.itemClickListener = { view, item, position ->
            when (item) {
                ...
                is RecyclerViewData.Ad -> {
                    sharedPreferences.edit {
                        putBoolean(PREFS_KEY_IS_AD_CANCELLED, false)
                    }
                    showAd = false
                    viewModel.getHomeListItems()
                }
        ...
        viewModel.mainListItemsLiveData.observe(this) { result ->
            when (result) {
                is Resource.Failure -> {
                ...
                is Resource.Success -> {
                    binding.progressBar.hide()
                    val list = result.value as MutableList
                    if (showAd) {
                        list.add(
                            RecyclerViewData.Ad(
                    ...
                    mainRecyclerViewAdapter.items = list
                }
            }
        }
    }
